    ...
    for i in range(TotalSbox - 1):
        ...
        if i < ActiveSbox:
            for j in range(i + 1):
                ...
                count_var_num += 1
        elif i >= TotalSbox - ActiveSbox:
            for j in range(i - TotalSbox + ActiveSbox + 1):
                auxiliary_var_u[i].append(0)
            for j in range(i - TotalSbox + ActiveSbox + 1, ActiveSbox):
                auxiliary_var_u[i].append(count_var_num)
                count_var_num += 1
        else:
...
for totalround in range(SearchRoundStart, SearchRoundEnd):
    ...
    if (GroupConstraintChoice == 1):
        for group in range(0, GroupNumForChoice1):
            for round in range(0, totalround - group):
