...
def Decision(Round, ActiveSbox, MatsuiRoundIndex, MatsuiCount, flag):
    ...
    for r in range(Round):
        ...
        inputxor4 = faout[r]
        for i in range(32):
            X = [inputxor1[i],inputxor2[i],inputxor21[i]]
            for k in range(4):
                clauseseq = genclause(X,k,2)
                file.write(clauseseq)
        for i in range(32):
            X = [inputxor3[i],inputxor4[i],inputxor21[i]]
            for k in range(4):
                clauseseq = genclause(X,k,2)
                file.write(clauseseq)
        ...
        inputxor8 = fdout[r]
        for i in range(32):
            X = [inputxor5[i], inputxor6[i], inputxor56[i]]
            for k in range(4):
                clauseseq = genclause(X, k, 2)
                file.write(clauseseq)
        for i in range(32):
            X = [inputxor7[i], inputxor8[i], inputxor56[i]]
            for k in range(4):
                clauseseq = genclause(X, k, 2)
                file.write(clauseseq)
        inputxor9 = xin[r][:32]
