...
for i in range(FullRound):
    DiffActiveSbox += [0]
res= [0,0,0,0,1,2,2,5,6,7,8,9,10,10,10,12,15,16]
for i in range(len(res)):
    DiffActiveSbox[i] = res[i]
...
for totalround in range(SearchRoundStart, SearchRoundEnd):
    ...
    while (flag == False):
        ...
        CountSbox += 1
    CountSbox -=1
    DiffActiveSbox[totalround] = CountSbox
